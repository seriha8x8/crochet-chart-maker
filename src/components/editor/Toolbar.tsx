...
export function Toolbar() {
  ...
  return (
    <div className="flex h-12 items-center justify-between border-b border-neutral-200 bg-white px-3">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-neutral-700">かぎ針編み図メーカー</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          ...
          {showGuidePopover && (
            <div className="absolute right-0 top-full z-40 mt-1 w-64 rounded-md border border-neutral-200 bg-white p-3 text-sm shadow-lg">
              <div className="mb-2 flex flex-col gap-1">
                {(["none", "chain", "ring"] as GuideType[]).map((t) => (
                  <label key={t} className="flex items-center gap-1.5">
                    <input type="radio" checked={guide.type === t} onChange={() => setGuide({ type: t })} />
                    {t === "none" ? "なし" : t === "chain" ? "鎖編みはじまり（直線）" : "輪はじまり（同心円）"}
                  </label>
                ))}
              </div>
              {guide.type === "chain" && (
                <div className="flex flex-col gap-1 border-t border-neutral-100 pt-2 text-xs">
